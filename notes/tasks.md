...
- [x] Согласие на обработку персональных данных (152-ФЗ): `ConsentLog` (guest_id, signed_at) — счётчик согласий, обязательный чекбокс + страница `/privacy` с политикой ПДн, ссылка в футере, счётчик на карточке гостя (478 тестов, rubocop 0)
...
- [x] Рефакторинг после миграции: `AppBase` (общий конфиг/error-хендлеры/parse_date/slice_params), admin.rb разбит на 6 модулей (`app/controllers/admin/*_routes.rb`), поиск номеров вынесен в `RoomSearch` + `Room#available_for_booking?`, удалён мёртвый код (POST toggle_vip, двойной authenticity_token в button_to) — 128 тестов, rubocop 0, smoke зелёный (подробности — [[decisions]])
- [x] Багфикс после аудита логики: 9 багов — freeze_prices (room=nil), RecordNotFound → 404, мультивыбор удобств (path_with массивы), бронь без ФИО → 422, belongs_to_required_by_default, next_free_window, checked_out не блокирует ночи, min_nights валидация, self-merge юзеров (коммит 4e2d86c, 128 тестов, rubocop 0, smoke зелёный — подробности — [[decisions]])
